...
async function ensureFailedPaymentsTable() {
  try {
    // Try to fetch from table first to see if it exists
    const records = await base('Failed Payments').select({ maxRecords: 1 }).firstPage();
    addLog('Failed Payments table already exists');
    return true;
  } catch (error) {
    if (error.message.includes('NOT_FOUND')) {
      addLog('Failed Payments table not found, it needs to be created manually in Airtable', 'warn');
      addLog('Please create a table named "Failed Payments" with fields: Payment ID, Customer Email, Amount, Currency, Failure Reason, Timestamp, Status', 'warn');
      return false;
    }
    addLog(`Error checking Failed Payments table: ${error.message}`, 'error');
    return false;
  }
}
...
app.post('/webhook', (req, res) => {
  ...
  try {
    ...
    addLog(`Webhook signature verification failed: ${error.message}`, 'error');
